...
def pinecone_call(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    rt = retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10), reraise=True)
...
def dynamodb_call(func: Callable[..., Any]) -> Callable[..., Any]:
    rt = retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=5), reraise=True)
    return rt(func)


def embedding_call(func: Callable[..., Any]) -> Callable[..., Any]:
    rt = retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=8), reraise=True)
    return rt(func)


def web_search_call(func: Callable[..., Any]) -> Callable[..., Any]:
    cb = circuit_breaker("ddg", failure_threshold=2, recovery_timeout=120)
    rt = retry(stop=stop_after_attempt(2), wait=wait_exponential(multiplier=1, min=1, max=5), reraise=True)
    return _compose(cb, rt)(func)
